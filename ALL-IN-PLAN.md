...
**STILL LEFT FOR YOU:**
- **Agent Engine: BLOCKED by a Google-side platform constraint, not by our code.** Token scopes were added (gateway now 200), but two deeper facts surfaced: (a) the remote MCP gateway is a curated Davis/entity toolset with NO `execute_dql`, so it can't run the DQL-first loop on the OTel-only tenant; (b) `gemini-3-flash-preview` serves ONLY from `global` (404 in us-central1) while Agent Engine's build backend doesn't run at `global` (consistent 500 at the build LRO; no Cloud Build created there). So the agent can't be built where Gemini 3 serves, and Gemini 3 isn't served where it can be built. We added a self-contained `DYNATRACE_MCP_MODE=inprocess` (no subprocess, no gateway, 6 tests) so the agent is fully Agent-Engine-ready; it will deploy once Gemini 3 is GA in an Agent-Engine region, Agent Engine supports `global`, or you point AUTOSRE_MODEL at a regionally-available GA model for that variant. No billable resource was left (verified `agent_engines.list()` == 0). Eligibility is unaffected (ADK + Gemini 3 on Vertex, live on Cloud Run).
